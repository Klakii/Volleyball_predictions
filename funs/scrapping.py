''' Following functions are used for scrapping data from publicly available information from official plusliga website '''
...
def get_match_stats(): 
    '''
    Scrap the match statistics (number of blocks, aces, reception quality etc.) for all matches for all teams in seasons between 2010 and 2022. Data for currently played season, 2023/2024 is scrapped each week.
    ...
    Examples
    --------
    >>> get_match_stats() scraps data for season between 2010/2011 and 2022/2023
    ...
    # for 2023 data is scrpaped each week, once all games from one round are played, code below will be used once the whole season is played to gather data for the whole season at once            
    # # 2023
...
def get_table_positions(year: list):
    '''
    Scrap the table (position of teams with 'big points', number of matches won/lost so far, number of sets won/lost etc.) for given years. Works for seasons between 2010 and 2022.
    ...
    Examples
    --------
    >>> get_table_positions([2010. 2013]) scraps table after each round in the 2010/2011 and 2013/2014 seasons
...
def refresh_match_stats():
    '''
    Scrap match statistics for the currently played season, in this case 2023, but only till the current round - data about the current round is stored in csv file.
    ...
    Examples
    --------
    >>> refresh_match_stats() downloads latest data for the currently played season
    ...
    temp_.to_csv("data/reference.csv", sep=',',index=False)
...
def refresh_table_positions():
    '''
    Scrap the table (position of teams with 'big points', number of matches won/lost so far, number of sets won/lost etc.) for currently played season, till the latest round - latest round number is stored in the csv file.
    ...
    Examples
    --------
    >>> refresh_table_positions() scraps table for currently played season
...
def get_player_data():
    '''
    Scrap the players statistics for each team for each season. Data includes personal characteristics such as height, weight etc. 
    ...
    Examples
    --------
    >>> get_player_data() scraps data for players for season between 2010 and 2023
...
def get_additional_player_data():
    '''
    Scrap the players statistics for each team for each season. Data includes personal characteristics such as height, weight etc. This function downloads data that was ommited by the get_player_data function.  
    ...
    Examples
    --------
    >>> get_additional_player_data() scraps data for players for season between 2010 and 2023
